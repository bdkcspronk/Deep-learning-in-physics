...
The Galaxy Zoo 2 dataset was preprocessed by cropping, resizing, and normalization using ConvNeXt-specific mean and standard deviation. 
Training was performed in two ways: on the full dataset (normal training) and with curriculum learning, where data was staged by label confidence.
In both cases, only the final layer was trained, with all other weights frozen to leverage pre-trained features.
The model was trained for 20 epochs, batch size 512, using Adam optimizer with a learning rate of $2\mathrm{e-}4$.
...
I fine-tuned a pre-trained ConvNeXt model on Galaxy Zoo 2 by updating only the final layer, efficiently adapting it to the new task.
...
For this project, I split the training data into three stages based on label confidence: high (\(>0.94\)), medium (0.85-0.94), and low (\(<0.85\)), each about 33\% of the data, or in other words: \(\sim20,000\) images.
